...
def process_bulk_enquiry(df_bulk, stock_type, adjustment_percent=0):
    ...
    for idx, row in df.iterrows():
        ...
        # Superseeded Info: If a part has a superseded version, display the new part number in the Supersedes column only.
        # Requirement: Do not replace original part number, do not add "Supersedes:" prefix.
        ...
        if remainder <= 0:
        ...
        elif superseded_part_data:
            # We SPLIT only if user wants automated fulfillment. 
            # But the requirement says "display the new part number in the Supersedes column".
            # If we split, we might confuse the S.No logic.
            # Let's keep it as one row but show Supersedes, or split with 1.1, 1.2?
            # User requirement: "Editing Capabilities: Users must be able to delete individual records or modify quantities"
            # It's better to keep the original request as is, but maybe the user DOES want split logic?
            # Reading carefully: "Add a Back Order column to display the difference between Requested_Qty and Allocated_Qty"
            # Back order = Requested - Allocated.
            
            # If we use superseded logic from previous implementation:
            # We'll split but keep the same S.No or suffix it.
            ...
            sup_price = superseded_part_data.get('price')
